...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!decodedToken.admin) {
      return Response.json({ error: 'Forbidden: Only admins can set admin claims' }, { status: 403 });
    }
    ...
    return Response.json({ success: true, message: `Admin claim set for user ${uid}` });
  } catch (error) {
